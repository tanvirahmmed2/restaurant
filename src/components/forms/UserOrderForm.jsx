            ...
            <div className='max-w-7xl mx-auto flex flex-col gap-16'>
                ...
                <div className='flex items-center gap-6'>
                    ...
                    <div>
                        ...
                        <p className='text-slate-400 text-[10px] font-black uppercase tracking-[0.3em] mt-2 ml-1'>Order Settlement & Details</p>
                    </div>
                </div>
                ...
                <div className='grid grid-cols-1 lg:grid-cols-12 gap-16 items-start'>
                        ...
                        <section className='space-y-8'>
                            <div className='flex items-center justify-between px-2'>
                                <div className='flex items-center gap-3'>
                                    ...
                                    <h2 className='text-2xl font-black text-slate-800 tracking-tight'>Order Review</h2>
                                </div>
                                <button onClick={() => clearCart()} className='text-[10px] font-black text-slate-300 hover:text-rose-500 uppercase tracking-widest transition-colors cursor-pointer'>Discard Cart</button>
                            </div>
                                    ...
                                    <div key={item.cartItemId} className='bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-2xl shadow-pink-900/[0.01] flex items-center gap-8 group hover:shadow-2xl hover:shadow-pink-900/[0.03] transition-all duration-500'>
                                        ...
                                        <div className='flex-1 flex flex-col justify-between py-1'>
                                            <div className='flex justify-between items-start gap-4'>
                                                <div>
                                                    <h3 className='font-black text-slate-900 text-xl tracking-tight leading-tight'>{item.title}</h3>
                                                ...
                                                <p className='font-black text-slate-900 text-xl tracking-tighter'>৳{item.salePrice || item.price}</p>
                                            </div>
                    ...
                    <div className='lg:col-span-5 sticky top-32 space-y-10'>
                        ...
                        <section className='bg-white p-10 rounded-[3rem] border border-slate-100 shadow-[0_30px_100px_rgba(0,0,0,0.06)] space-y-10'>
                            <div className='space-y-6'>
                                <div className='flex items-center gap-3'>
                                    ...
                                    <h2 className='text-xl font-black text-slate-800 tracking-tight'>Fulfillment</h2>
                                </div>
                                <div className='grid grid-cols-2 gap-5'>
                                    {deliveryOptions.map((opt) => (
                                        ...
                                        >
                                            ...
                                            <span className={`text-[10px] font-black uppercase tracking-[0.2em] ${formData.delivery_method === opt.id ? 'text-black' : 'text-slate-300'}`}>{opt.label}</span>
                                        </button>
                                    ))}
                                </div>
                            </div>
                            ...
                            <div className='space-y-6 border-t border-slate-50 pt-10'>
                                ...
                                <div className='grid grid-cols-3 gap-3'>
                                    {paymentOptions.map((opt) => (
                                        ...
                                        >
                                            ...
                                            <span className={`text-[9px] font-black uppercase tracking-tighter text-center ${formData.payment_method === opt.id ? 'text-black' : 'text-slate-300'}`}>{opt.label}</span>
                                        </button>
                                    ))}
                                </div>
                                ...
                                {formData.payment_method !== 'cash' && (
                                    <div className='flex flex-col gap-2 animate-in fade-in slide-in-from-top-2'>
                                        <label className='text-[10px] font-black uppercase text-slate-400 tracking-[0.2em] ml-1'>Transaction Ref</label>
                                        <input 
                                            type="text" 
                                            name='transaction_id' 
                                            required 
                            ...
                            <div className='space-y-4 border-t border-slate-50 pt-10'>
                                ...
                                <div className='flex justify-between text-4xl font-black text-slate-900 pt-6 border-t border-dashed border-slate-100'>
                                    <span>Total</span>
